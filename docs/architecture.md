...
`go-log` only provides handlers and writers that compose those types.
...
shut down telemetry. The separate `go-telemetry` runtime owns those concerns and
returns standard contexts and APIs.

This direction prevents dependency cycles: `go-telemetry` does not need
`go-log`, and `go-log/otel` does not need the `go-telemetry` runtime module.
